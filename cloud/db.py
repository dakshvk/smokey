...
get(det_id) - Fetches one detection by id
...
def insert(det: dict) -> bool:
    ...
    with connect() as db:
        cur = db.execute(
            ...
            json.dumps(det['bbox']) if det.get('bbox') else None))
        return cur.rowcount == 1 # returns if it was actually inserted if ==1 returns true

def get(det_id: str) -> dict | None:
    '''Fetches one detection by id - used before attaching its image.'''
    with connect() as db:
        return db.execute('SELECT * FROM detections WHERE id = %s', (det_id,)).fetchone()

def set_image_key(det_id: str, key: str) -> bool:
    '''Attaches the stored image location to a detection'''
    with connect() as db:
        cur = db.execute('UPDATE detections SET image_key = %s WHERE id = %s', (key, det_id))
...
def recent(limit: int = 50, camera:str | None = None) -> list[dict]:
    ...
    with connect() as db:
        return db.execute(sql, args + (limit,)).fetchall() # connect to postrge then execute and gives us the limited query
